...
float speed2 = 0.90;        // Scenario 1: bus2 (blue) individual speed
bool  bus1On = true;        // Scenario 1: red bus on/off
bool  bus2On = true;        // Scenario 1: blue bus on/off
...
// Flock B: then 3 ti kore birds jabe
...
// Flock C:then 2ti kore birds jabe
...
glBegin(GL_QUADS);
   ...
   ///1st floor design
    ...
    glEnd();
       ...
       ///2nd floor design ///y+55 add koresi
    ...
    ///3rd floor design ///y+55 add koresi
    ...
    ///4th floor design ///y+55 add koresi
    ...
    /// 5th floor design ///y+55 add koresi
...
glPushMatrix();
    ...
    glColor3ub(217, 217, 217);// tower circle shape korar jonno
    ...
    glTranslatef(-273, -500, 0);
    glBegin(GL_LINES);
        glColor3ub(140, 140, 140);//pakha asbe
        glVertex2f(269, 500);
        glVertex2f(277, 500);
        glVertex2f(273, 504);
        glVertex2f(273, 496);
...
///white part///
    ...
    ///Hospital lekha asbe tar kaj hoise
...
glColor3ub(253, 183, 77);    ///S U N color///
    circle(18,36,0,0);
glPopMatrix();
}
//ob23
...
// BEZIER CURVE - Scenario-1 draw hobe
...
void drawBezierBridge()
{
    ...
    // 5 ti curve bridge add hobe (beizer)
    float archPts[6][4] = {
        {0,   160},
        {160, 320},
        {320, 480},
...
// SCENARIO 4 GLOBALS (Smart City - Fog/Beach Scene)
...
int s4_triangleAmount = 1000;
GLfloat s4_twicePi = 2.0f * PI;

float s4_vehicleSpeed = .003f;
bool  s4_night    = false;
bool  s4_on       = false;
bool  s4_vehicleStop = false;
bool  s4_rain     = false;
bool  s4_waveUp   = true;

float s4_crashB2 = 0.0f;
float s4_crashB1 = 0.0f;

float s4_rain1x=0,s4_rain2x=0,s4_rain3x=0,s4_rain4x=0,s4_rain5x=0;
float s4_rain6x=0,s4_rain7x=0,s4_rain8x=0,s4_rain9x=0,s4_rain10x=0,s4_rain11x=0;

float s4_rain1y=0,s4_rain2y=0,s4_rain3y=0,s4_rain4y=0,s4_rain5y=0;
float s4_rain6y=0,s4_rain7y=0,s4_rain8y=0,s4_rain9y=0,s4_rain10y=0;
float s4_rain11y=0,s4_rain12y=0,s4_rain13y=0,s4_rain14y=0,s4_rain15y=0;

int s4_i = 0;

float s4_run1=0,s4_run2=0,s4_run3=0,s4_run4=0,s4_run5=0;
float s4_run6=0,s4_run7=0,s4_run8=0,s4_run9=0,s4_run10=0,s4_run11=0;

float s4_c1=0,s4_c2=0,s4_c3=0,s4_c4=0,s4_c5=0,s4_c6=0,s4_c7=0;
float s4_c8=0,s4_c9=0,s4_c10=0,s4_c11=0,s4_c12=0,s4_c13=0;
float s4_c14=0,s4_c15=0,s4_c16=0,s4_c17=0;


    void s4_resetRain()//raindrops back to their initial state
    {
        s4_rain1y = 0.0;
        s4_rain2y = 0.0;
        s4_rain3y = 0.0;
    ...
    void s4_clearColor(float r, float g, float b,float bb)//set the background color of the screen
    {
        glClearColor(r,g,b,bb);
    }

    void s4_renderBitmapString(float x, float y, float z, void *font, const char *string) //rendering refers to the process of generating an image or a scene from a 3D model
    {
        const char *c;
        glRasterPos3f(x, y,z); //sets the position in 3D space
        for (c=string; *c != '\0'; c++)//
        {
            glutBitmapCharacter(font, *c);//renders a single character from the string at the current raster position.
        }
    }
        void s4_hill1(){
        //Hill-N015
            glPushMatrix(); //// Save the current transformation matrix
            glTranslatef(0.3f,0.0f,0.0f);//// Apply a translation transformation

            glBegin(GL_POLYGON);//// Start defining a polygon
            glColor3f(0.0f,0.2f,0.0f);// Set the color for the polygon (dark green)
        ...
        void s4_hill2(){
            ...
            if(s4_night) //// If 's4_night' is true, change the color to a darker green
            {
        ...
        void s4_hill3(){//hill-N015

            glPushMatrix();//// Save the current transformation matrix
            glTranslatef(0.78f,0.0f,0.0f); // Apply a translation transformation along the x-axis
        ...
        //hill-N015
            ...
            glEnd();}
        void s4_leftTree(){  // tree-N017


            glPushMatrix();
            glTranslatef(0.06f, -0.01f, 0.0f);//// Apply translation to position the tree
            float s4_run3 = 0.0;
            glBegin(GL_QUADS);
            ...
            GLfloat xt10=-0.955f;
            GLfloat yt10=0.3f;
            GLfloat radiust10 =0.03f;//first circular layer of leaves.
            glColor3f(0.0f,0.2f,0.0f);
            glBegin(GL_TRIANGLE_FAN);//Starts drawing a circle
            glVertex2f(xt10, yt10);
            for(int i = 0; i <= s4_triangleAmount; i++)   //Circle Formula in Polar Coordinates:
            {
                glVertex2f( xt10 + (radiust10* cos(i *  s4_twicePi / s4_triangleAmount)),
                            yt10 + (radiust10 * sin(i * s4_twicePi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xt11 + (radiust11 * cos(i *  s4_twicePi / s4_triangleAmount)),
                            yt11 + (radiust11 * sin(i * s4_twicePi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xt12 + (radiust12 * cos(i *  s4_twicePi / s4_triangleAmount)),
                            yt12 + (radiust12 * sin(i * s4_twicePi / s4_triangleAmount)) );
            }
            glEnd();
        ...
        void s4_leftBuilding(){//Buldings-N018
            ...
            glColor3f(0.6f,0.8f,1.0f);//// Set the color to a light blue
            if (s4_night)
            {
                glColor3f(0.9f,0.9f,0.0f);//If it's s4_night, set the color to yellow
            }
            ...
            glEnd();
            glLineWidth(1);// // Set the line width to 1 (normal thickness)
            glBegin(GL_LINES);
            glVertex2f(-0.80f, 0.32f);//first line
            glVertex2f(-0.80f, 0.38f);//second line
            glEnd();
        ...
        void s4_secondBuilding(){//Buldings-N018
            ...
            glLineWidth(6);// Set the line width to 6 pixels
            glBegin(GL_LINES);
            ...
            glBegin(GL_QUADS);
            glColor3f(0.38, 0.68, 0.78);//// Set color to a light blue-green
            if (s4_night)
            {
                glColor3f(0.9f,0.9f,0.0f);// If it's s4_night, change color to yellow
            }
        ...
        void s4_carForSecondBuilding(){// car-N021
        ...
        void s4_thirdBuilding(){//buldings-N018
            ...
            // Render the string "Royal" at the given position
            s4_renderBitmapString(-0.168f, 0.35f, 0.0f, GLUT_BITMAP_HELVETICA_10, "Royal");
        ...
        /*void carForFourthBuilding(){
            //car-N021

            glPushMatrix(); // Save the current transformation matrix
            glTranslatef(0.445f, 0.055f, 0.0f);// Apply translation (move the object)
            glScalef(.7,.7,0);// Apply scaling (reduce the size)
            ...
            glLoadIdentity();// Reset the current matrix (typically used to reset transformations)

            glPopMatrix();

        }*/
       void s4_fourthBuilding()
{
    glPushMatrix();
    ...
    glPopMatrix();
        ...
        //buldings-N018
            ...
            glLineWidth(6); //This function sets the width (thickness) of the lines drawn in OpenGL.

                            //The value 6 means that all subsequent lines will be drawn with a
            glBegin(GL_LINES);
            ...
            glVertex2f(0.37f, 0.35);
            glEnd();*/
            glLoadIdentity();//resets any previous transformations.
            glPopMatrix();
        ...
        void s4_sky(){//SKY-N011

            glBegin(GL_QUADS);

            glColor3f(0.4f,0.69f,1.0f);// Set the color to a light blue (sky color)

            if(s4_rain && !s4_night)  // Check if it's raining and not s4_night
            {
                glColor3f(0.18, 0.36, 0.56);// Set the color to a darker blue (cloudy or rainy sky)
            }
            glVertex2f(-1.0f, 0.5f);
            glVertex2f(1.0f, 0.5f);
            ...
            if(s4_night)
            {
                glColor3f(0.2, 0.2, 0.40);// Set the color to dark blue (s4_night sky)
                glVertex2f(-1.0f, 0.5f); // Bottom-left corner
                glVertex2f(-1.0f, 1.0f);// Top-left corner
                glColor3f(0.0, 0.01, 0.02); // Dark s4_night color
        ...
        void s4_sun(){
            ...
            if(!s4_night)
            {
                GLfloat xs1=0.2f;// x-coordinate of the sun's center
                GLfloat ys1=0.85f;// y-coordinate of the sun's center

                GLfloat radiuss1 =0.05f;// Radius of the sun (size)
                glColor3f(0.95f,0.95f,0.0); // Set the color to a yellow (sunny color)
                glBegin(GL_TRIANGLE_FAN);// Start defining a circle using triangle fan
                glVertex2f(xs1, ys1);// Define the center of the circle (the sun's center)


                for(int i = 0; i <= s4_triangleAmount; i++)
                {
                    glVertex2f( xs1 + (radiuss1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                                ys1 + (radiuss1 * sin(i * s4_twicePi / s4_triangleAmount)) );
                }
                glEnd();
            }
        ...
        // moon-N013
            if(s4_night)
            {
                GLfloat xsm1=0.78f;// x-coordinate of the moon's center
                GLfloat ysm1=0.9f;// y-coordinate of the moon's center

                GLfloat radiussm1 =0.05f; // Radius of the moon
                glColor3f(1.0f,1.0f,1.0);// Set color to white (moon color)

                glBegin(GL_TRIANGLE_FAN);
                glVertex2f(xsm1, ysm1);// Define the center of the moon


                // Loop to define the points on the shadow circle
                for(int i = 0; i <= s4_triangleAmount; i++)
                {
                    glVertex2f( xsm1 + (radiussm1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                                ysm1 + (radiussm1 * sin(i * s4_twicePi / s4_triangleAmount)) );
                }
                glEnd();
                ...
                for(int i = 0; i <= s4_triangleAmount; i++)
                {
                    glVertex2f( xsm2 + (radiussm1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                                ysm2 + (radiussm1 * sin(i * s4_twicePi / s4_triangleAmount)) );
                }
                glEnd();

            }
        }
        void s4_star(){    // moon-N013
            if(s4_night)
            {
                GLfloat xsm1=0.78f;
                GLfloat ysm1=0.9f;
        ...
        void s4_cloud(){//cloud-N016......................
            glPushMatrix();
            glTranslatef(s4_run1,0,0);// Apply translation to the first cloud

            glPushMatrix();
            glTranslatef(0.05f, 0.14f, 0.0f);
            glScalef(.7,.8,0);// Scale the cloud (make it slightly smaller and more oval-shaped)

            GLfloat xm1=-0.8f;
            GLfloat ym1=0.9f;
            GLfloat radiusm1 =0.08f;
            glColor3f(0.95f,0.95f,0.95); // Default color (white)
            if(s4_night)
            {
                glColor3f(0.5f,0.5f,0.5);// Darker color for the cloud if it's s4_night
            }
            if(s4_rain && !s4_night)
            {
                glColor3f(0.43, 0.63, 0.85); // Cloud color for rainy daytime
            }
            glBegin(GL_TRIANGLE_FAN);
            glVertex2f(xm1, ym1);// Center of the cloud
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xm1 + (radiusm1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                            ym1 + (radiusm1 * sin(i * s4_twicePi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xm2 + (radiusm1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                            ym2 + (radiusm1 * sin(i * s4_twicePi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xm3 + (radiusm2 * cos(i *  s4_twicePi / s4_triangleAmount)),
                            ym3 + (radiusm2 * sin(i * s4_twicePi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xm4 + (radiusm2 * cos(i *  s4_twicePi / s4_triangleAmount)),
                            ym4 + (radiusm2 * sin(i * s4_twicePi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xm5 + (radiusm2 * cos(i *  s4_twicePi / s4_triangleAmount)),
                            ym5 + (radiusm2 * sin(i * s4_twicePi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xcl1 + (radiusm1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                            ycl1 + (radiusm1 * sin(i * s4_twicePi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xcl2 + (radiusm4 * cos(i *  s4_twicePi / s4_triangleAmount)),
                            ycl2 + (radiusm4 * sin(i * s4_twicePi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xcl3 + (radiusm5 * cos(i *  s4_twicePi / s4_triangleAmount)),
                            ycl3 + (radiusm5* sin(i * s4_twicePi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xcl4 + (radiusm6* cos(i *  s4_twicePi / s4_triangleAmount)),
                            ycl4 + (radiusm6* sin(i * s4_twicePi / s4_triangleAmount)) );
            }
            glEnd();
            glLoadIdentity();
            glPopMatrix();
        ...
        void s4_lampUpSide()
{
    ...
    glPopMatrix();
        ...
        void s4_bus(){
            ...
            glPushMatrix();
            glTranslatef(s4_run4,0,0);// Move the bus horizontally based on s4_run4
            GLfloat xb1=-0.455f;
            GLfloat yb1=0.095f;
            GLfloat radiusb1 =.01f;

            glColor3f(1.0, 0.0, 0.0); // Set wheel color to red
            glBegin(GL_TRIANGLE_FAN);
            glVertex2f(xb1, yb1);
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xb1 + (radiusb1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                            yb1 + (radiusb1 * sin(i * s4_twicePi / s4_triangleAmount)));
            }
            glEnd();
            ...
            {
                glBegin(GL_QUADS);
                glColor3f(1.0f,0.6f,0.2f);// Light yellow color
                glVertex2f(-0.445f, 0.097f);
                glVertex2f(-0.455f, 0.095f);
                glColor3f(1.0f,1.0f,0.79f);// Brighter yellow
            ...
            glBegin(GL_QUADS);
            glColor3f(1.0, 0.5, 0.0); // Orange color for the bus body
            glVertex2f(-.45,.188);
            glVertex2f(-.45,.06);
            glColor3f(0.98, 0.88, 0.02); // Yellow for the lower part
            glVertex2f(-.6,.06);
            glVertex2f(-.6,.188);
            glEnd();
            ...
            glLineWidth(6);
            glBegin(GL_LINES);
            glColor3f(0.76, 0.77, 0.78);// Light gray color for the window borders
            glVertex2f(-.455f,.17f);
            glVertex2f(-.455f,.12f);
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xb2 + (radiusb2 * cos(i *  s4_twicePi / s4_triangleAmount)),
                            yb2 + (radiusb2 * sin(i * s4_twicePi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xb3 + (radiusb3 * cos(i *  s4_twicePi / s4_triangleAmount)),
                            yb3 + (radiusb3 * sin(i * s4_twicePi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xb4 + (radiusb2 * cos(i *  s4_twicePi / s4_triangleAmount)),
                            yb4 + (radiusb2 * sin(i * s4_twicePi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xb5 + (radiusb3 * cos(i *  s4_twicePi / s4_triangleAmount)),
                            yb5 + (radiusb3 * sin(i * s4_twicePi / s4_triangleAmount)) );
            }
            glEnd();
        ...
        void s4_lamp2()
{
    ...
    glPopMatrix();
        ...
        void s4_bus2(){// bus-N023

            glPushMatrix();

            glTranslatef(s4_run7,0,0);// Apply translation to move the bus horizontally based on s4_run7
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xb6 + (radiusb6 * cos(i *  s4_twicePi / s4_triangleAmount)),
                            yb6 + (radiusb6 * sin(i * s4_twicePi / s4_triangleAmount)));
            }
            glEnd();

            if(s4_night)
            {
                glBegin(GL_QUADS);// Draw the light effect on the bus
                glColor3f(1.0f,0.6f,0.2f); // Light yellow color
                glVertex2f(-0.445f, 0.097f);
                glVertex2f(-0.455f, 0.095f);
                glColor3f(1.0f,1.0f,0.79f); // Brighter yellow
            ...
            glBegin(GL_QUADS);
            glColor3f(0.0, 0.4, 0.0); // Dark green for the upper body
            glVertex2f(-.45,.188);
            glVertex2f(-.45,.06);
            glColor3f(1.0, 0.2, 0.2); // Red for the lower body
            glVertex2f(-.6,.06);
            glVertex2f(-.6,.188);
            glEnd();
            ...
            glLineWidth(6); // Set line width
            glBegin(GL_LINES);
            glColor3f(0.76, 0.77, 0.78);// Light gray color
            glVertex2f(-.455f,.17f);
            glVertex2f(-.455f,.12f);
            glEnd();
            ...
            glPushMatrix();
            glTranslatef(-.022,0,0);// Translate for the next window
            glBegin(GL_QUADS);
            glColor3f(0.76, 0.77, 0.78);// Light gray color for window grills
            glVertex2f(-.465,.17);
            glVertex2f(-.465,.14);
            glVertex2f(-.48,.14);
            glVertex2f(-.48,.17);
            ...
            glTranslatef(-.044,0,0);
            glBegin(GL_QUADS);
            glColor3f(0.76, 0.77, 0.78);// Black color for the headlight
            glVertex2f(-.465,.17);
            glVertex2f(-.465,.14);
            glVertex2f(-.48,.14);
            glVertex2f(-.48,.17);
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xb7 + (radiusb7 * cos(i *  s4_twicePi / s4_triangleAmount)),
                            yb7 + (radiusb7 * sin(i * s4_twicePi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xb8 + (radiusb8 * cos(i *  s4_twicePi / s4_triangleAmount)),
                            yb8 + (radiusb8 * sin(i * s4_twicePi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xb9 + (radiusb9 * cos(i *  s4_twicePi / s4_triangleAmount)),
                            yb9 + (radiusb9 * sin(i * s4_twicePi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xb10 + (radiusb10 * cos(i *  s4_twicePi / s4_triangleAmount)),
                            yb10 + (radiusb10* sin(i * s4_twicePi / s4_triangleAmount)) );
            }
            glEnd();
     ...
     //sea............................................................................................
        ...
        glPushMatrix();// Save the current matrix state to the stack.
        glTranslatef(0,-.1,0);// Apply translation to the object.
        glBegin(GL_QUADS);
        glColor3f(0.0f,0.29f,0.6f);// Set the color for the quad (a shade of blue).
        if(s4_night)
        {
            glColor3f(0.0f, 0.09f, 0.4f);// Darker blue for s4_night-time effect
        }
       ...
       //c1..............................
        glPushMatrix(); // Save the current transformation matrix.
        glTranslatef(0,s4_c1,0); // Translate the object (move it along the y-axis by 'c1' units).
        GLfloat xw1=-0.9f; // x-coordinate of the center of the first circle.
        GLfloat yw1=-0.61f;// y-coordinate of the center of the first circle.
        GLfloat radiusw1 =0.3f; // Radius of the circle.
        glColor3f(0.0f,0.29f,0.6f); // Set color to a shade of blue.
        if(s4_night)
        {
            glColor3f(0.0f, 0.09f, 0.4f);// If it's s4_night, change the color to a darker blue.
        }
        glBegin(GL_TRIANGLE_FAN); // Start drawing a circle using GL_TRIANGLE_FAN.
        glVertex2f(xw1, yw1); // Center of the circle.

         // Loop to create vertices for the circle.
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xw1 + (radiusw1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yw1 + (radiusw1 * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xw2 + (radiusw1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yw2 + (radiusw1 * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        glPopMatrix();
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xw3 + (radiusw1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yw3 + (radiusw1 * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        glPopMatrix();
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xw4 + (radiusw1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yw4 + (radiusw1 * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        glPopMatrix();
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xw5 + (radiusw1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yw5 + (radiusw1 * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        glPopMatrix();
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xw6 + (radiusw1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yw6 + (radiusw1 * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        glPopMatrix();
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xw7 + (radiusw1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yw7 + (radiusw1 * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        glPopMatrix();
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xw8 + (radiusw1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yw8 + (radiusw1 * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        glPopMatrix();
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xw9 + (radiusw1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yw9 + (radiusw1 * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        glPopMatrix();
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xw10 + (radiusw1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yw10 + (radiusw1 * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        glPopMatrix();
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xw11 + (radiusw1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yw11 + (radiusw1 * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        glPopMatrix();
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xw12 + (radiusw1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yw12 + (radiusw1 * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        glPopMatrix();
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xw13 + (radiusw1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yw13 + (radiusw1 * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        glPopMatrix();
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xw14 + (radiusw1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yw14 + (radiusw1 * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        glPopMatrix();
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xw15 + (radiusw1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yw15 + (radiusw1 * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        glPopMatrix();
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xw16 + (radiusw1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yw16 + (radiusw1 * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        glPopMatrix();
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xw17 + (radiusw1 * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yw17 + (radiusw1 * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        glPopMatrix();
        ...
        // small.boat....................................................
        glPushMatrix();// Save the current transformation matrix
        glTranslatef(0,-.1,0);// Translate the boat downward by 0.1 units
        if(!s4_night && ! s4_rain)
        {
            glPushMatrix();
            glTranslatef(s4_run8,0,0); // Translate the boat horizontally by s4_run8 units (movement along x-axis)

            glPushMatrix();
            glTranslatef(0.01f, -0.2f, 0.0f);
            glScalef(.5,.5,0); // Scale the boat to 50% of its original size in both x and y directions


            glBegin(GL_POLYGON);

            glColor3f(0.4f,0.0f,0.0f);// Set the color of the boat to dark red

            // Define the vertices for the boat's hull (quadrilateral shape)
            ...
            // Draw the boat's base (green rectangle)
            glBegin(GL_QUADS);
            glColor3f(0.0f,0.6f,0.0f); // Set the base color to green
            ...
            glEnd(); // End the base drawing


            // Draw the boat's mast (black triangle)
            glBegin(GL_TRIANGLES);
            glColor3f(0.0f,0.0f,0.0f); // Set the mast color to black
            ...
            glLoadIdentity();// Reset the transformation to default
            glPopMatrix(); // Restore previous transformation
            glPopMatrix(); // Restore the boat's transformation
        }
        glPopMatrix();

        //smallboat s4_night
        if(s4_night || s4_rain)// Check if it's s4_night or raining
        {
            glPushMatrix();
            glTranslatef(1.1,-.05,0); // Translate the boat horizontally

            glPushMatrix();// Save transformation state for the next object
            glTranslatef(0.01f, -0.2f, 0.0f);// Position the boat
            glScalef(.5,.5,0);// Scale the boat size by 50%


            // Draw the boat hull (polygon)
            glBegin(GL_POLYGON);

            glColor3f(0.4f,0.0f,0.0f);// Dark red color
            glVertex2f(-0.75f, -0.47f);
            glVertex2f(-0.65f, -0.47f);
            glVertex2f(-0.6f, -0.4f);
            glVertex2f(-0.65f, -0.47f);
            ...
            glBegin(GL_QUADS); // Draw the boat's base (green rectangle)
            glColor3f(0.0f,0.6f,0.0f); // Green color for the base
            ...
            // Draw the boat's mast (black triangle)
            glBegin(GL_TRIANGLES);
            glColor3f(0.0f,0.0f,0.0f);// Black color for the mast
        ...
        //small boat copy...........................................................
        glPushMatrix();
        glTranslatef(0,-.1,0); // Save the current transformation state (matrix).
        if(!s4_night && !s4_rain)// If it's neither s4_night nor s4_rain
        {
            glPushMatrix();
            glTranslatef(s4_run9,0,0);// Translate the boat horizontally by s4_run9 units (for animation or movement).

            glPushMatrix();
            glTranslatef(0.09f,-0.3f, 0.0f);
            glRotatef(180,0,1,0); // Rotate the boat by 180 degrees around the y-axis (flip horizontally).
            ...
            //// Define the vertices for the boat hull.
            glColor3f(0.2f,0.1f,0.0f);// Set the color to a dark brown.
            glVertex2f(-0.75f, -0.47f);
            glVertex2f(-0.65f, -0.47f);
            glVertex2f(-0.6f, -0.4f);
            glVertex2f(-0.65f, -0.47f);
            ...
            glColor3f(0.9f,0.5f,0.0f); // Set color to orange for the boat base.

            // Define the vertices of the base rectangle.
            glVertex2f(-0.72f, -0.39f);
            glVertex2f(-0.67f, -0.39f);
            glVertex2f(-0.65f, -0.435f);
            glVertex2f(-0.7f, -0.435f);
            ...
            glBegin(GL_TRIANGLES);
            glColor3f(0.0f,0.0f,0.0f);// Draw the mast (black triangle)
        ...
        //s4_night
            if(s4_night || s4_rain)
            {
                ...
                glColor3f(0.2f,0.1f,0.0f);// Set color to dark brown

            glVertex2f(-0.75f, -0.47f);  // Bottom left point
            glVertex2f(-0.65f, -0.47f);  // Bottom right point
            glVertex2f(-0.6f, -0.4f);   // Upper right point
            glVertex2f(-0.65f, -0.47f);  // Closing the bottom right side of the hull
            glVertex2f(-0.8f, -0.4f);    // Upper left point
            glVertex2f(-0.75f, -0.47f);  // Closing the bottom left side of the hull
            glVertex2f(-0.6f, -0.4f);    // Upper right point
            glVertex2f(-0.65f, -0.44f);  // Upper right close point
            glVertex2f(-0.75f, -0.44f);  // Upper left closing point
            glVertex2f(-0.8f, -0.4f);    // Upper left point
            glVertex2f(-0.65f, -0.44f);  // Upper right close point
            glVertex2f(-0.75f, -0.44f);  // End the upper left closing point
            glEnd();  // End of drawing the boat hull

            glColor3f(0.9f,0.5f,0.0f);// Set the base color to orange

            // Define the vertices of the base rectangle
            glVertex2f(-0.72f, -0.39f);  // Bottom left of the base
            glVertex2f(-0.67f, -0.39f);  // Bottom right of the base
            glVertex2f(-0.65f, -0.435f); // Top right of the base
            glVertex2f(-0.7f, -0.435f);  // Top left of the base
            glEnd();  // End of drawing the base
            ...
            // Define the vertices of the mast triangle
            glVertex2f(-0.72f, -0.39f);  // Left point of the mast
            glVertex2f(-0.7f, -0.43f);   // Right point of the mast
            glVertex2f(-0.75f, -0.418f); // Peak of the mast
            glEnd();  // End of drawing the mast
        ...
        glPushMatrix();
        glTranslatef(s4_run10,0,0); // Move the boat horizontally based on s4_run10

        glPushMatrix();
        glTranslatef(0.01f,-0.3f, 0.0f);
        glScalef(.7f,0.6f,0.0f);// Scale the boat (larger in width and smaller in height)

        // Drawing the hull of the boat
        glBegin(GL_QUADS);
        glColor3f(0.6f,0.29f,0.0f);

        glColor3f(0.6f, 0.29f, 0.0f);  // Set the hull color to a dark brown
        glVertex2f(0.1f, -0.8f);   // Bottom left point
        glVertex2f(0.5f, -0.8f);   // Bottom right point
        glVertex2f(0.4f, -0.9f);   // Top right point
        glVertex2f(0.2f, -0.9f);   // Top left point

        glColor3f(0.0f, 0.0f, 0.0f);  // Set color to black for the next part
        glVertex2f(0.6f, -0.8f);   // Bottom left point of the black part
        glVertex2f(0.5f, -0.8f);   // Bottom right point
        glVertex2f(0.4f, -0.9f);   // Top right point
        glVertex2f(0.5f, -0.9f);   // Top left point

        glColor3f(1.0f, 1.0f, 1.0f);  // Set color to white for this part
        glVertex2f(0.18f, -0.88f);  // Bottom left point
        glVertex2f(0.2f, -0.9f);   // Bottom right point
        glColor3f(0.0f, 0.0f, 0.0f);  // Set color back to black
        glVertex2f(0.4f, -0.9f);   // Top right point
        glVertex2f(0.42f, -0.88f);  // Top left point

        // Additional parts of the boat (base, other features)
        glColor3f(0.5f, 0.5f, 0.5f);  // Gray color for the boat base
        glVertex2f(0.15f, -0.75f);  // Bottom left point
        glVertex2f(0.45f, -0.75f);  // Bottom right point
        glVertex2f(0.45f, -0.8f);   // Top right point
        glVertex2f(0.15f, -0.8f);   // Top left point

        // White base color for another section
        glColor3f(1.0f, 1.0f, 1.0f);
        glVertex2f(0.55f, -0.75f);  // Bottom left point
        glVertex2f(0.45f, -0.75f);  // Bottom right point
        glVertex2f(0.45f, -0.8f);   // Top right point
        glVertex2f(0.55f, -0.8f);   // Top left point

        // Blue section of the boat (upper side)
        glColor3f(0.8f, 0.6f, 1.0f);  // Light blue color
        glVertex2f(0.15f, -0.75f);  // Bottom left point
        glVertex2f(0.45f, -0.75f);  // Bottom right point
        glVertex2f(0.4f, -0.7f);    // Top right point
        glVertex2f(0.15f, -0.7f);   // Top left point

        glColor3f(0.6f, 0.6f, 1.0f);  // Slightly darker blue
        glVertex2f(0.55f, -0.75f);  // Bottom left point
        glVertex2f(0.45f, -0.75f);  // Bottom right point
        glVertex2f(0.4f, -0.7f);    // Top right point
        glVertex2f(0.5f, -0.7f);    // Top left point
        glEnd();  // End drawing the hull


        /**.................................... big boat-Pillar .......................................**/

        glColor3f(0.0f,0.0f,0.0f);// Black color for the pillars
        glBegin(GL_QUADS);
        glVertex2f(0.18f, -0.65f);  // Bottom left point of pillar 1
        glVertex2f(0.21f, -0.65f);  // Bottom right point
        glVertex2f(0.21f, -0.7f);   // Top right point
        glVertex2f(0.18f, -0.7f);   // Top left point
        glEnd();  // End pillar drawing

        // Draw the brown part of the pillars
        glColor3f(0.6f, 0.29f, 0.0f);  // Brown color for the upper part of the pillars
        glBegin(GL_QUADS);
        glVertex2f(0.18f, -0.65f);  // Bottom left point of pillar 1
        glVertex2f(0.21f, -0.65f);  // Bottom right point
        glVertex2f(0.21f, -0.63f);  // Top right point
        glVertex2f(0.18f, -0.63f);  // Top left point
        glEnd();  // End drawing the brown part
        ...
        glColor3f(0.6f,0.8f,1.0f);// Set the sail color (light blue)
        if (s4_night)
        {
            glColor3f(0.9f,0.9f,0.0f);// Change sail color to yellow if it's s4_night
        }
        glBegin(GL_QUADS);
        ...
        glPushMatrix();
        glTranslatef(0.05f, 0.0f, 0.0f); // Move the sails horizontally
        glColor3f(0.6f,0.8f,1.0f);
        if (s4_night)
        {
            glColor3f(0.9f,0.9f,0.0f);
        ...
        GLfloat radiusb =.015f;// Circle radius for decoration
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xboat1 + (radiusb * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yboat1 + (radiusb * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xboat2 + (radiusb * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yboat2 + (radiusb * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xboat3 + (radiusb * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yboat3 + (radiusb * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xboat4 + (radiusb * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yboat4 + (radiusb * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xboat5 + (radiusb * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yboat5 + (radiusb * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        ...
        for(int i = 0; i <= s4_triangleAmount; i++)
        {
            glVertex2f( xboat6 + (radiusb * cos(i *  s4_twicePi / s4_triangleAmount)),
                        yboat6 + (radiusb * sin(i * s4_twicePi / s4_triangleAmount)) );
        }
        glEnd();
        ...
        glPopMatrix();
            ...
            void s4_displayScene()
            {
                s4_clearColor(0.0f, 0.7f, 0.4f, 1.0f); // Set background color to a greenish shade (daytime)
                if(s4_night)
                {
                    s4_clearColor(0.07, 0.30, 0.15, 1);// If it's s4_night time, change the background color to a darker green
                }

                glClear(GL_COLOR_BUFFER_BIT);    // Clear the color buffer (remove previous frames to avoid overlap)
            ...
            s4_leftTree();
            s4_secondBuilding();
            s4_carForSecondBuilding();
            s4_thirdBuilding();

            s4_fourthBuilding();
//            carForFourthBuilding();
            s4_fifthBuilding();
//            sixthBuilding();
            s4_bus();
        ...
        //////tree-N017
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xf1 + (radiusf1 * cos(i *  twice1Pi / s4_triangleAmount)),
                            yf1 + (radiusf1 * sin(i * twice1Pi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xf2 + (radiusf2 * cos(i *  twice1Pi / s4_triangleAmount)),
                            yf2 + (radiusf2 * sin(i * twice1Pi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xf3 + (radiusf3 * cos(i *  twice1Pi / s4_triangleAmount)),
                            yf3 + (radiusf3 * sin(i * twice1Pi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xf4 + (radiusf4 * cos(i *  twice1Pi / s4_triangleAmount)),
                            yf4 + (radiusf4 * sin(i * twice1Pi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xf5 + (radiusf5 * cos(i *  twice1Pi / s4_triangleAmount)),
                            yf5 + (radiusf5 * sin(i * twice1Pi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xf6 + (radiusf6 * cos(i *  twice1Pi / s4_triangleAmount)),
                            yf6 + (radiusf6 * sin(i * twice1Pi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xf7 + (radiusf7 * cos(i *  twice1Pi / s4_triangleAmount)),
                            yf7 + (radiusf7 * sin(i * twice1Pi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xf8 + (radiusf8 * cos(i *  twice1Pi / s4_triangleAmount)),
                            yf8 + (radiusf8 * sin(i * twice1Pi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xf9 + (radiusf9 * cos(i *  twice1Pi / s4_triangleAmount)),
                            yf9 + (radiusf9 * sin(i * twice1Pi / s4_triangleAmount)) );
            }
            glEnd();
            glLoadIdentity();
            glPopMatrix();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xf10 + (radiusf10* cos(i *  twice1Pi / s4_triangleAmount)),
                            yf10 + (radiusf10 * sin(i * twice1Pi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xf11 + (radiusf11 * cos(i *  twice1Pi / s4_triangleAmount)),
                            yf11 + (radiusf11 * sin(i * twice1Pi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xf12 + (radiusf12 * cos(i *  twice1Pi / s4_triangleAmount)),
                            yf12 + (radiusf12 * sin(i * twice1Pi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xf13 + (radiusf13* cos(i *  twice1Pi / s4_triangleAmount)),
                            yf13 + (radiusf13 * sin(i * twice1Pi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xf14 + (radiusf14 * cos(i *  twice1Pi / s4_triangleAmount)),
                            yf14 + (radiusf14 * sin(i * twice1Pi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xf15 + (radiusf15 * cos(i *  twice1Pi / s4_triangleAmount)),
                            yf15 + (radiusf15 * sin(i * twice1Pi / s4_triangleAmount)) );
            }
            glEnd();
        ...
        //buldings-N018
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xt1 + (radiust1 * cos(i *  twice1Pi / s4_triangleAmount)),
                            yt1 + (radiust1 * sin(i * twice1Pi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xt2 + (radiust2 * cos(i *  twice1Pi / s4_triangleAmount)),
                            yt2 + (radiust2 * sin(i * twice1Pi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            for(int i = 0; i <= s4_triangleAmount; i++)
            {
                glVertex2f( xt3 + (radiust3 * cos(i *  twice1Pi / s4_triangleAmount)),
                            yt3 + (radiust3 * sin(i * twice1Pi / s4_triangleAmount)) );
            }
            glEnd();
            ...
            if(s4_rain)
            {
                // First s4_rain layer
                glPushMatrix();
                glTranslatef(s4_rain1x,s4_rain1y,0);// Translate the s4_rain to its position
                glBegin(GL_LINES);// Begin drawing lines (raindrops)
                glColor3f(1,1,1);// Set the color of the s4_rain to white

                // Drawing vertical lines to represent individual raindrops
                glVertex2f(-.95,.9);
                glVertex2f(-.955,.85);
                glVertex2f(-.85,.9);
                glVertex2f(-.855,.85);
                ...
                // Duplicate layer of s4_rain with a small horizontal shift
                glPushMatrix();
                glTranslatef(-.05,0,0);// Translate the s4_rain slightly left
                glBegin(GL_LINES);
                glColor3f(1,1,1); // White color for the s4_rain
                ...
                glEnd();
                glPopMatrix();// Pop the s4_rain shift matrix
                glPopMatrix();// Pop the first s4_rain matrix

                //rain2 <<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

                glPushMatrix();
                glTranslatef(s4_rain3x,s4_rain3y,0);
                glPushMatrix();
                ...
                glPushMatrix();
                glTranslatef(s4_rain5x,s4_rain5y,0);
                ...
                glPushMatrix();
                glTranslatef(s4_rain7x,s4_rain7y,0);
                ...
                glPushMatrix();
                glTranslatef(s4_rain9x,s4_rain9y,0);
            ...
            //render-"render" means to display or draw something on the screen.
            glColor3f(0.0,0.0,0.0);// Render empty space (in black)
            s4_renderBitmapString(0.52f, 0.5f, 0.0f, GLUT_BITMAP_9_BY_15, "");//display text on the screen using bitmap font

            //glColor3f(0.0,0.0,0.0);// Render "BULDING-01" in white at the specified position
            //s4_renderBitmapString(0.811f, 0.3f, 0.0f, GLUT_BITMAP_HELVETICA_10, "BULDING-01");


            glColor3f(0.6,0.0,0.0);// Render "BULDING-02" in dark red at the specified position
            s4_renderBitmapString(-0.85f, 0.4f, 0.0f, GLUT_BITMAP_HELVETICA_10, "BULDING-01");

            glColor3f(1.0,1.0,1.0);// Render "BULDING-03" in white at the specified position
            s4_renderBitmapString(-0.59f, 0.61f, 0.0f, GLUT_BITMAP_HELVETICA_10, "BULDING-02");

            glColor3f(0.0,0.2,0.0);// Render "CASTLE" in dark green at the specified position
            s4_renderBitmapString(0.11f, 0.375f, 0.0f, GLUT_BITMAP_HELVETICA_10, "             CASTLE          ");
        ...
        void s4_update(int value)  // Update function to animate the objects and handle the animation states
        {
            // Update vehicle positions
            {
                s4_run11 += .003f;// Update horizontal movement

                if (s4_run11 > 1.25)
            {
                s4_run11 = -1;  // Reset the position of the object when it moves too far
            }

            }


            if(s4_on) // If the animation is on
            {
                if(!s4_vehicleStop)
                {
                    s4_run7 -= s4_vehicleSpeed;// Move the vehicle
                    if(!s4_vehicleStop)
                    {
                        s4_run4 += s4_vehicleSpeed;
                    }
                    s4_run5 += (s4_vehicleSpeed+.001);
                    s4_run6 -= (s4_vehicleSpeed+.001);
                    s4_run3 -= (s4_vehicleSpeed+.002);
                }
                 // Update other elements (like moving clouds, buildings, etc.)
                s4_run1 += .0003f;
                s4_run2 -= .0002f;
                s4_run10 += .0009f;

               // Handle s4_rain animation by moving raindrops downward
               if(s4_rain)  //s4_rain animation
                {
                    s4_rain1y -= .05;
                    s4_rain2y -= .05;
                    s4_rain3y -= .05;
                ...
                // If it's neither s4_night nor raining, animate the movement of objects
                if(!s4_night && !s4_rain)
                {
                    s4_run8 -= .0005f;// Move the object represented by 's4_run8' to the left
                    s4_run9 += .0005f;// Move the object represented by 's4_run9' to the right

                }
            }
            if(s4_waveUp)  // Additional updates for wave movement
            {
                s4_c1 += .00042;
                s4_c2 += .00042;
                s4_c3 += .00042;
            ...
            // Reset position of the object if it moves too far left
            if (s4_run7 < -1.7)
            {
                s4_run7 = .5; // Reset to starting position on the right
            }
            if (s4_run5 > 1)
            {
                s4_run5 = -1.2;
            ...
            // Rain animation reset checks
            if(s4_rain1y < -1.9)
            {
                s4_rain1y = .1;// Reset s4_rain position to start from the top
                s4_rain1x = 0;
            }
            if(s4_rain2y < -1.7)
            {
            ...
            // Prevent the wave from exceeding a certain height
            if (s4_c1 >= .08)
        {
            s4_waveUp = false;
        }
        ...
        if (s4_c17 < 0.0)
        {
            ...
            glutPostRedisplay();  // Post a redisplay for the next update
            glutTimerFunc(25, s4_update, 0);// Calls update function every 25 ms
        }


// ============================================================
// SCENARIO 4: Smart City (Beach/Fog Scene)
// ============================================================
void displayScenario4()
{
    // Switch to normalized projection for Scenario 4
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
...
// ============================================================
// UNIFIED DISPLAY MAIN (4 Scenarios)
// ============================================================
void displayMain()
{
...
// SPECIAL KEY HANDLER (Scenario 4 vehicle speed)
...
void handleSpecialKey(int key, int x, int y)
{
    if (currentScenario == 4)
    {
        ...
        {
        case GLUT_KEY_UP:
            s4_vehicleStop = false;
            s4_vehicleSpeed += 0.001f;
            break;
        case GLUT_KEY_DOWN:
            if(s4_vehicleSpeed > 0) s4_vehicleSpeed -= 0.001f;
            if(s4_vehicleSpeed <= 0) { s4_vehicleSpeed = 0; s4_vehicleStop = true; }
            break;
        }
    }
    glutPostRedisplay();
...
// UNIFIED KEYBOARD HANDLER kaj hoise (all 4 scenarios)

void handleKeypress(unsigned char key, int x, int y)
{
    // --- Scenario Switch Keys ---
    // '9','8','7' switch from any scenario (no conflict with Scenario 4 keys)
    ...
    if (key == '7') { currentScenario = 4; glutPostRedisplay(); return; }
    ...
    if (currentScenario == 1 || currentScenario == 12)
    {
        switch (key)
        {
        // Red bus (bus1) controls
        case 'q': bus1On = !bus1On; break;          // Red bus ON/OFF toggle
        case 'w': speed += 0.3f; break;             // Red bus speed up
        case 'e': speed -= 0.3f; if(speed<0) speed=0; break; // Red bus speed down

        // Blue bus (bus2) controls
        case 'a': bus2On = !bus2On; break;          // Blue bus ON/OFF toggle
        case 's': speed2 += 0.3f; break;            // Blue bus speed up
        case 'd': speed2 -= 0.3f; if(speed2<0) speed2=0; break; // Blue bus speed down
    ...
    // --- Scenario 4 Keys (original fog_fixed controls preserved) ---
    if (currentScenario == 4)
    {
        switch (key)
        {
        case 'n': s4_night = true;  break;
        case 'd': s4_night = false; break;
        case '1': s4_on = true;     break;
        case '2': s4_on = false;    break;
        case 'r': s4_rain = true;   break;
        case 'e': s4_rain = false;  break;
        case 'x': exit(0);          break;
        }
    }
...
// UNIFIED MOUSE HANDLER kaj hoise(all 4 scenarios)
...
// MAIN // shift hobe 1 scene hote onno scene button er maddhome oi kaj hoise
...
int main(int argc, char** argv)
{
    ...
    glutCreateWindow("A BEAUTIFUL CITY  |  0=City  9=River City  8=Train & Bridge  7=Smart City");
